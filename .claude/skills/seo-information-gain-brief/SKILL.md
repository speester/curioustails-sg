...
Files read:
- `business-context.md` — business facts, differentiation, brand rules,
  ...
  **Regulatory/health facts override everything:** if business-context.md has a
  ...
  where it gets in.
- `expertise-library.md` — first-party stories/numbers/quotes for P1 sections.
...
### Priority levels
- **P1** — original/first-party (from expertise-library.md/ground-truth)
...
- Showcase/inventory section BEFORE educational content (for transactional
  intent) — place this as the first H2 unless business-context.md specifies otherwise.
...
- Care/usage/practical-reality content per business-context.md guidance.
- A lifestyle-fit or suitability section — match by real use-case/lifestyle,
  not by a proxy variable, unless business-context.md says otherwise.
...
- Health/risk topics (if relevant): handle through the lens of the business's
  ...
  mentions per business-context.md's stated limit (default: once, if unspecified).

**Phase 4 — Social Proof, Risk Reversal, Final Objection Clearance**
- Testimonials/reviews/case studies (use real, consented material from expertise-library.md).
- Guarantee/aftercare/post-purchase protection — wording must match
  business-context.md exactly (e.g. if it says "assistance and care" rather
  than "we pay vet fees," use that framing verbatim).
- A value/investment breakdown (what's included, quantified) — only build a
  head-to-head price comparison table if business-context.md explicitly wants
...
- Trust/licence verification section — place late in the page unless
  business-context.md says otherwise.
...
- Any licensing/paperwork/fee timing note should match business-context.md
...
23. ≥2 sections include a first-person "Practitioner's Note" key_point — a
    ...
    drawn from expertise-library.md (never fabricated).
...
{ heading, level (H2/H3), priority (P1-P4/P4-TERMINAL), cro_phase (1-5),
  ...
  from expertise-library.md; leave "" otherwise), external_link_cue }
```
...
- [ ] Business-specific policy wording (health guarantee framing, price-table
      ...
      business-context.md exactly — nothing borrowed from an unrelated template.
