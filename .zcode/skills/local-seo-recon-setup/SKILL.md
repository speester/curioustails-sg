---
name: local-seo-recon-setup
description: One-time setup checklist for the local-seo-recon skill. Installs Python deps (requirements.txt), configures DataForSEO API credentials as env vars, lists DataForSEO location codes, and optionally installs the Lighthouse CLI and Chrome first-fold screenshots. Triggers when the user asks to "set up recon", "install recon deps", "configure DataForSEO", or is preparing to run local-seo-recon for the first time.
---
